...
-- ADDED: Safe Zone Variables  
local safeZoneConnection = nil  
local safeZonePlatform = nil  
local FollowTarget = nil -- Stores a Player object for safezone and follow commands  
local SAFE_ZONE_OFFSET = Vector3.new(0, 15, 0) -- Example offset, adjust as needed  
...
local function addToWhitelist(playerName)  
    ...
    if player then  
        if not table.find(Whitelist, player.Name) then  
            ...
            sendMessage(player.Name .. " whitelisted. They will not be affected by aura.") -- Added confirmation message  
            return true  
        end  
        sendMessage(player.Name .. " is already whitelisted.") -- Added message for already whitelisted  
    end  
    sendMessage("Could not find player " .. playerName .. " to whitelist.") -- Added message for player not found  
    return false  
end  
    ...
    if player then  
        for i, whitelistName in ipairs(Whitelist) do  
            if whitelistName == player.Name then  
                ...
                sendMessage(player.Name .. " unwhitelisted.") -- Added confirmation message  
                return true  
            end  
        end  
        sendMessage(player.Name .. " was not in the whitelist.") -- Added message for not in whitelist  
    end  
    sendMessage("Could not find player " .. playerName .. " to unwhitelist.") -- Added message for player not found  
    return false  
end  
...
local function addTarget(playerName)  
    ...
    if player then  
        if not table.find(Targets, player.Name) then  
            ...
            startForceEquip()  
            sendMessage(player.Name .. " added to targets.") -- Added confirmation message  
            return true  
        end  
        sendMessage(player.Name .. " is already a target.") -- Added message for already targeted  
    end  
    sendMessage("Could not find player " .. playerName .. " to target.") -- Added message for player not found  
    return false  
end  
    ...
    if player then  
        for i, targetName in ipairs(Targets) do  
            if targetName == player.Name then  
                ...
                sendMessage(player.Name .. " removed from targets.") -- Added confirmation message  
                return true  
            end  
        end  
        sendMessage(player.Name .. " was not in the target list.") -- Added message for not in targets  
    end  
    sendMessage("Could not find player " .. playerName .. " to remove from targets.") -- Added message for player not found  
    return false  
end  
...
local function setAura(range)  
    ...
    if newRange and newRange >= 0 then  
        ...
        sendMessage("Aura range set to " .. newRange .. ".") -- Added confirmation message  
        return true  
    end  
    sendMessage("Invalid aura range. Please provide a non-negative number.") -- Added error message  
    return false  
end  

-- ADDED: stopSafeZoneLoop function  
local function stopSafeZoneLoop()  
    ...
    FollowTarget = nil  
    sendMessage("Safezone deactivated.") -- Confirmation message  
end  
...
local function onMessageReceived(message)  
    ...
    if command == "connect" then  
        ...
        elseif player == MainConnector and args[2] then  
            ...
            if targetPlayer and not isConnected(targetPlayer) then  
                ...
                sendMessage("Could not find " .. args[2] .. " or already connected.") -- Added message  
            end  
        end  
        return  
    end  
        ...
        if targetPlayer and targetPlayer ~= MainConnector then  
            for i, connectedUser in ipairs(ConnectedUsers) do  
                if connectedUser == targetPlayer then  
                    ...
                    sendMessage("Unconnected: " .. targetPlayer.Name .. ".") -- Added message  
                    break  
                end  
            end  
        else  
            sendMessage("Could not unconnect " .. args[2] .. ".") -- Added message  
        end  
        return  
    end  
    ...
    if command == ".loop" then  
        if args[2] then  
            if args[2]:lower() == "all" then  
                sendMessage("Looping all non-whitelisted players.")  
    ...
    elseif command == ".unloop" then  
        if args[2] then  
            if args[2]:lower() == "all" then  
                sendMessage("Unlooping all targets.")  
                table.clear(Targets)  
                stopForceEquip()  
                unequipSword()  
            else  
    ...
    elseif command == ".aura" then  
        if args[2] then  
            ...
            elseif args[2]:lower() == "off" then -- Added .aura off command  
                setAura(0)  
            else  
                setAura(args[2])  
            end  
    ...
    elseif command == ".reset" then  
        ...
        sendMessage("Attempting to rejoin game.") -- Added message  
    ...
    elseif command == ".refresh" then  
        ...
        if char then  
            ...
            if hrp and humanoid then  
                ...
                -- Kill the character  
                humanoid.Health = 0  
                sendMessage("Character refreshed.") -- Added message  
            else  
                sendMessage("Could not refresh character.") -- Added message  
            end  
        end  
    -- ADDED: Safezone commands  
    elseif command == ".safezone" and args[2] then  
        local targetPlayer = findPlayer(args[2])  
        if not (targetPlayer and LP.Character and LP.Character.PrimaryPart) then  
            sendMessage("Could not set safezone. Target player or your character not found.")  
            return  
        end  
        stopSafeZoneLoop() -- Stop any existing safezone loop  
        ...
        safeZonePlatform.Parent = game:GetService("Workspace") -- Parent after setting properties  
        ...
        safeZoneConnection = RunService.Heartbeat:Connect(function()  
            if not (FollowTarget and FollowTarget.Character and FollowTarget.Character.PrimaryPart and LP.Character and LP.Character.PrimaryPart and safeZonePlatform and safeZonePlatform.Parent) then  
                sendMessage("Safezone target or self lost. Disabling safezone.")  
                stopSafeZoneLoop()  
                return  
            end  
            ...
            -- Assuming a teleportTo function exists or directly setting CFrame  
            if LP.Character and LP.Character.PrimaryPart then  
                 LP.Character:SetPrimaryPartCFrame(CFrame.new(myNewPos) * (myHRP.CFrame - myHRP.CFrame.Position))  
            end  
        end)  
        sendMessage("Safezone set around " .. targetPlayer.Name .. ".")  
...
LP.CharacterAdded:Connect(function(c)  
    ...
    -- ADDED: Stop safezone loop on character added (respawn)  
    stopSafeZoneLoop()  
end)  
...
Players.PlayerRemoving:Connect(function(p)  
    ...
    if MainConnector == p then  
        ...
        sendMessage("Main Connector has left. Connection reset.") -- Added message  
    end  
    ...
    -- ADDED: Stop safezone loop if the followed target leaves  
